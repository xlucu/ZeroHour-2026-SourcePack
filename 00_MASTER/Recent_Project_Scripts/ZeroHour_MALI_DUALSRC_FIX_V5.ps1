...
try {
    ...
    Banner "4/7 AUTO-DETECT GRADLE WRAPPER AND BUILD APK ONLY"
    ...
    if ([string]::IsNullOrWhiteSpace($gradlew)) {
        Log "Standard Gradle wrapper locations not found. Searching project tree..."
    ...
    if ([string]::IsNullOrWhiteSpace($gradlew)) {
        ...
        if ($null -ne $gradleCmd) {
            $gradlew = [string]$gradleCmd.Source
            $useSystemGradle = $true
            Log "No wrapper found. Using system Gradle: $gradlew"
        }
        else {
            Fail "No gradlew.bat found anywhere under project root, and no system Gradle command is available."
        }
    }
    else {
        Log "SELECTED GRADLE WRAPPER: $gradlew"
    }

    $settingsGroovy = Join-Path $Android "settings.gradle"
    $settingsKts    = Join-Path $Android "settings.gradle.kts"

    if (-not (Test-Path $settingsGroovy) -and -not (Test-Path $settingsKts)) {
        Log "WARNING: No settings.gradle/settings.gradle.kts directly under $Android"
        Log "Searching for Android Gradle project root containing app..."
        ...
        foreach ($s in $settingsCandidates) {
            ...
            if (Test-Path (Join-Path $candidateDir "app")) {
                ...
                Log "GRADLE PROJECT CANDIDATE: $projectDir"
                break
            }
        }

        if ([string]::IsNullOrWhiteSpace($projectDir)) {
            Fail "Could not locate a Gradle project directory containing both settings.gradle(.kts) and app."
        }
    }
    else {
        $projectDir = $Android
    ...
    Log "GRADLE PROJECT DIR: $projectDir"

    Push-Location $projectDir
    try {
        Run-Native -Exe $gradlew -ArgList @("-p", $projectDir, ":app:assembleDebug") -Label "Gradle assembleDebug"
    }
    finally {
        Pop-Location
    }

    $expectedApk = Join-Path $projectDir "app\build\outputs\apk\debug\app-debug.apk"
    ...
    if (-not (Test-Path -LiteralPath $Apk)) {
        Fail "Gradle finished but app-debug.apk was not found."
    }
