...
static void drawButtonText( GameWindow *window, WinInstanceData *instData )
{
	...
	text->setWordWrapCentered( BitTestEA( instData->getStatus(), WIN_STATUS_WRAP_CENTERED ));
	...
	// where to draw
	if( BitTestEA( window->winGetStatus(), WIN_STATUS_SHORTCUT_BUTTON ) )
	{
																	 ...
																	 WinInstanceData *instData )
{
	...
	{
		if( BitTestEA( instData->getState(), WIN_STATUS_USE_OVERLAY_STATES ) )
		{
			ICoord2D size, start;
			// get window position
																	 ...
																	 WinInstanceData *instData )
{
	...
	if( !BitTestEA( window->winGetStatus(), WIN_STATUS_USE_OVERLAY_STATES ) )
	{
	...
	if( image )
	{
		...
		{	
			...
			{
				if( !BitTestEA( window->winGetStatus(), WIN_STATUS_NOT_READY ) )
				{
					...
					if( !BitTestEA( window->winGetStatus(), WIN_STATUS_ALWAYS_COLOR ) )
					{
	...
	if( BitTestEA( window->winGetStatus(), WIN_STATUS_FLASHING ) )
	{
	...
	if( BitTestEA( window->winGetStatus(), WIN_STATUS_USE_OVERLAY_STATES ) )
	{
		...
		if( pushedOverlayIcon && hilitedOverlayIcon )
		{
			if(BitTestEA(window->winGetStatus(), WIN_STATUS_ENABLED))
			{
  			...
  			else if( BitTestEA( instData->getState(), WIN_STATE_SELECTED ) )
 				{
