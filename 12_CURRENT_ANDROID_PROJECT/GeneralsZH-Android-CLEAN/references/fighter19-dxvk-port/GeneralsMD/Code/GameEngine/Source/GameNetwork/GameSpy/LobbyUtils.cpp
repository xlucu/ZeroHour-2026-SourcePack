													...
													UnsignedInt mouse)
{
	...
	Int gameID = (uintptr_t)GadgetListBoxGetItemData(window, row, 0);
...
void RefreshGameListBox( GameWindow *win, Bool showMap )
{
	...
	if (selectedIndex != -1 )
	{
		selectedID = (uintptr_t)GadgetListBoxGetItemData(win, selectedIndex);
	}
...
//	}
//
//	Int selectedID = (uintptr_t)GadgetListBoxGetItemData(mainWin, selected);
//	if (selectedID < 0)
//	{
//		return;
//	}
...
void playerTemplateComboBoxTooltip(GameWindow *wndComboBox, WinInstanceData *instData, UnsignedInt mouse)
{
	...
	Int templateNum = (uintptr_t)GadgetComboBoxGetItemData(wndComboBox, index);
...
void playerTemplateListBoxTooltip(GameWindow *wndListBox, WinInstanceData *instData, UnsignedInt mouse)
{
	...
	Int templateNum = (uintptr_t)GadgetListBoxGetItemData(wndListBox, row, col);
	UnicodeString ustringTooltip;
	if (templateNum == -1)
	{
			// the "Random" template is always first
