																											 ...
																											 Player *thePlayer)
{
	...
	for( them = iter->first(); them; them = iter->next() )
	{
		...
		if( BitTestEA( options, IGNORE_STEALTHED ) )
		{
			if( rel != ALLIES )
			{
				...
				{
					if( BitTestEA( options, FAIL_STEALTHED_WITHOUT_FEEDBACK ) )
					{
																											 ...
																											 Player *player)
{
	...
	if( BitTestEA( options, CLEAR_PATH ) && builderObject && !builderObject->isKindOf( KINDOF_IMMOBILE ) )
	{
